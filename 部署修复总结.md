...
### ❌ 问题 6：TypeScript rootDir 限制
**错误信息**：`error TS6059: File is not under 'rootDir'. 'rootDir' is expected to contain all source files.`

**原因**：`packages/server/tsconfig.json` 设置了 `rootDir: "./src"`，限制了只能编译 src 目录下的文件，但代码引用了外部文件（games/avalon 和 packages/shared）

**修复**：删除 `tsconfig.json` 中的 `rootDir` 配置

**文件**：`packages/server/tsconfig.json` 第 7 行
...
- [x] ✅ packages/server/tsconfig.json - 删除 rootDir 限制 ⭐ **新增**
...
| packages/server/tsconfig.json | 删除 rootDir 限制 ⭐ **新增** | 7 |
